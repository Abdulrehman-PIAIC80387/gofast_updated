...
def list_Travel_search(request):
	...
	if request.method == 'POST':
		...
		startdate = request.POST.get('startdate', None)
		enddate = request.POST.get('enddate', None)
		customer_name = request.POST.get('custumer_name', None)
		print(customer_name)


		print(startdate)
		print(enddate)
		queryset = Invoice.objects.raw('select * from invoice_app_expense where date between "'+startdate+'" and "'+enddate+'"')
