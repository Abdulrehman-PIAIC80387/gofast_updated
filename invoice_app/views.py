...
def multiple_record(request):
	...
	def get(self, request, *args, **kwargs):
		...
		pdf = render_to_pdf('multiple_report.html', context)
		return HttpResponse(pdf, content_type='application/pdf')
	pdf = render_to_pdf('multiple_report.html', context)
	return HttpResponse(pdf, content_type='application/pdf')
...
def profit_calculations(request):
	
	
	month = request.POST.get('month')
	year = request.POST.get('year')
	received_monthly = Invoice.objects.filter(date__month=month).aggregate(Sum("received"))['received__sum'] or 0.00
	pending_monthly = Invoice.objects.filter(date__month=month).aggregate(Sum("pending"))['pending__sum'] or 0.00
	amount_monthly = Expense.objects.filter(date__month=month).aggregate(Sum("amount"))['amount__sum'] or 0.00
	received_profit = Invoice.objects.filter(date__month=month).aggregate(Sum("profit"))['profit__sum'] or 0.00
	net_profit = received_profit - amount_monthly
	received_yearly = Invoice.objects.filter(date__year=year).aggregate(Sum("received"))['received__sum'] or 0.00
	pending_yearly = Invoice.objects.filter(date__year=year).aggregate(Sum("pending"))['pending__sum'] or 0.00
	amount_yearly = Expense.objects.filter(date__year=year).aggregate(Sum("amount"))['amount__sum'] or 0.00
	received_profit_yearly = Invoice.objects.filter(date__year=year).aggregate(Sum("profit"))['profit__sum'] or 0.00
	net_profit_yearly = received_profit_yearly - amount_yearly
	
    

	print("i am in profit-------------------------------")
	print(month)
	print(year)
	print(received_monthly)
	print(pending_monthly)
	print(amount_monthly)
	print(received_profit)
	print(received_profit_yearly)
	print(amount_yearly)
	print(pending_yearly)
	print(received_yearly)
	print(net_profit)
	print(net_profit_yearly)
...
'received_yearly':received_yearly,
'pending_yearly':pending_yearly,
'amount_yearly':amount_yearly,
'received_profit_yearly':received_profit_yearly,
'net_profit_yearly':net_profit_yearly,
	...
	return render(request, "profit_calculations.html",context)
